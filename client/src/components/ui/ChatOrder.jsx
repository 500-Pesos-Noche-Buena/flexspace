...
const ChatOrder = ({ onClose, onOrderComplete }) => {
    ...
    useEffect(() => {
        if (isAuthenticated) {
            fetchActiveBookingAndProducts();
        }
    }, [isAuthenticated]);

    // Fetch user's active booking and the space
    const fetchActiveBookingAndProducts = async () => {
        setLoading(true);
        try {
            // Fetch user's active bookings (confirmed, active, ready status)
            const bookingsRes = await apiGet('/user/bookings');
            
            if (bookingsRes.success && bookingsRes.data) {
                const bookings = bookingsRes.data.bookings || [];
                
                // Find active booking (confirmed, active, or ready)
                const active = bookings.find(booking => 
                    ['confirmed', 'active', 'ready', 'preparing'].includes(booking.status)
                );
                
                if (active) {
                    setActiveBooking(active);
                    setActiveSpace(active.space_id);
                    console.log('📍 Active booking found:', active.ticket_number, 'at', active.space_id?.name);
                } else {
                    // No active booking - show message
                    showToast({ 
                        icon: 'warning', 
                        title: 'No Active Session', 
                        text: 'You need an active booking to order food. Please book a space first.' 
                    });
            ...
            // Fetch products (only show available products)
    ...
    const handleCheckout = () => {
        if (!isAuthenticated) {
            showToast({ icon: 'warning', title: 'Login Required', text: 'Please login to place an order' });
            onClose();
            return;
        }
        if (!activeSpace) {
    ...
    const processOrder = async () => {
        ...
        try {
            const orderData = {
                ...
                space_id: activeSpace._id,  // IMPORTANT: Use the active booking space
                booking_id: activeBooking?._id  // Link to the booking
            };
    ...
    // If not authenticated or no active booking, show nothing
    if (!isAuthenticated) return null;
    
    // Show loading or no active booking state
    if (loading) {
        return (
    ...
    return (
        <>
            ...
            <div className="fixed inset-0 bg-black/70 flex items-center justify-center z-10000 p-2 sm:p-4" style={{ zIndex: 10001 }}>
                <div className="bg-[#0f0f12] rounded-2xl border border-white/10 w-full max-w-4xl max-h-[95vh] sm:max-h-[90vh] overflow-hidden flex flex-col">
                    ...
                    <div className="flex flex-col md:flex-row flex-1 overflow-hidden">
                        {/* Products Panel - Scrollable on mobile */}
                        <div className="flex-1 p-3 sm:p-4 overflow-y-auto max-h-[50vh] md:max-h-none">
                            <div className="mb-3">
                                <input
                                    type="text"
                                    ...
                                    className="w-full px-3 sm:px-4 py-2 rounded-xl bg-white/5 border border-white/10 text-white focus:border-indigo-500 outline-none text-sm"
                                />
                            </div>
                            ...
                            <div className="flex gap-1.5 sm:gap-2 overflow-x-auto mb-3 sm:mb-4 pb-2">
                                {categories.map(cat => (
                                    <button
                                        ...
                                        className={`px-2.5 sm:px-3 py-1.5 rounded-full text-[10px] sm:text-xs font-bold whitespace-nowrap transition-all ${selectedCategory === cat.id
                        ...
                        {/* Cart Panel - Fixed height on mobile */}
                        <div className="w-full md:w-80 bg-[#1a1a24] border-t md:border-t-0 md:border-l border-white/10 flex flex-col max-h-[40vh] md:max-h-none">
                            <div className="p-3 sm:p-4 border-b border-white/10">
                                <div className="flex justify-between items-center">
                                    ...
                                    {cart.length > 0 && (
                                        <button onClick={() => setCart([])} className="text-red-400 text-[10px] sm:text-xs hover:text-red-300">
                                            Clear
                                        </button>
                                    )}
                                </div>
                                        ...
                                        <div key={item.id} className="bg-white/5 rounded-xl p-2">
                                            ...
                                            <div className="flex justify-between items-center mt-1 sm:mt-2">
                                                <div className="flex gap-1.5 sm:gap-2">
                                                    <button onClick={() => updateQuantity(item.id, -1)} className="w-4 h-4 sm:w-5 sm:h-5 rounded-lg bg-white/10 hover:bg-white/20 flex items-center justify-center">
                                                        <Minus size={8} sm:size={10} />
                                                    </button>
                                                    <span className="text-white text-[10px] sm:text-xs w-5 sm:w-6 text-center">{item.quantity}</span>
                                                    <button onClick={() => updateQuantity(item.id, 1)} className="w-4 h-4 sm:w-5 sm:h-5 rounded-lg bg-white/10 hover:bg-white/20 flex items-center justify-center">
                                                        <Plus size={8} sm:size={10} />
                                                    </button>
                                                </div>
                            ...
                            {cart.length > 0 && (
                                <div className="p-2 sm:p-3 border-t border-white/10">
                                    ...
                                    <button
                                        ...
                                        className="w-full mt-2 sm:mt-3 py-1.5 sm:py-2 bg-indigo-600 hover:bg-indigo-500 rounded-xl text-white font-bold text-[10px] sm:text-xs"
                                    >
                                        Checkout
                                    </button>
                                </div>
            ...
            {/* Payment Method Modal - Mobile Responsive */}
            {paymentModal && (
            ...
            {/* Payment QR Modal - Mobile Responsive */}
            {showPaymentQR && (
                        ...
                        {paymentStatus === 'pending' ? (
                            <>
                                ...
                                <div className="flex gap-2 sm:gap-3">
                                    <button
                                        ...
                                        className="flex-1 py-1.5 sm:py-2 bg-indigo-600/20 text-indigo-400 rounded-xl text-[9px] sm:text-xs font-bold"
                                    >
                                        Copy Link
                                    </button>
                                    <button
                                        onClick={() => window.open(paymentLink, '_blank')}
                                        className="flex-1 py-1.5 sm:py-2 bg-purple-600 hover:bg-purple-500 rounded-xl text-white text-[9px] sm:text-xs font-bold"
                                    >
                                        Open Link
                                    </button>
                                </div>
                            ...
                            <div className="py-6 sm:py-8">
                                ...
                                <p className="text-slate-400 text-xs sm:text-sm mb-3 sm:mb-4">
                                    ₱{calculateTotal().toFixed(2)} confirmed.
                                </p>
                                <p className="text-emerald-400 text-[10px] sm:text-sm">Your order is now being prepared.</p>
                            </div>
                        ) : paymentStatus === 'failed' ? (
                            <div className="py-6 sm:py-8">
                                <div className="w-12 h-12 sm:w-16 sm:h-16 mx-auto bg-red-500/20 rounded-full flex items-center justify-center mb-3 sm:mb-4">
                                    <XCircle size={24} sm:size={32} className="text-red-400" />
                                </div>
                                <h3 className="text-white font-black text-base sm:text-lg mb-1 sm:mb-2">Payment Failed</h3>
                                <p className="text-slate-400 text-xs sm:text-sm mb-3 sm:mb-4">
                                ...
                                <button
                                    ...
                                    className="w-full py-1.5 sm:py-2 bg-purple-600 hover:bg-purple-500 rounded-xl text-white font-bold text-xs sm:text-sm"
                                >
                                    Try Again
                                </button>
                            </div>
