...
import { FileText, BadgeCheck, Trash2, Edit3, ShieldCheck } from 'lucide-react';
...
const SpaceManagement = () => {
    ...
    const handleDelete = async (id) => {
        ...
        if (result.isConfirmed) {
            try {
                // Adjust this endpoint if you added a destroy method to SpaceController
                ...
                {selectedOwner && (
                    <div className="space-y-5 py-2">
                        ...
                        <div className="flex justify-end gap-3 pt-6 border-t border-white/5">
                            <button onClick={() => setOpenModal(false)} className="px-6 py-2.5 rounded-xl text-[10px] font-black uppercase text-slate-500 hover:text-white transition-all">Cancel</button>
                            <button onClick={handleSave} className="px-8 py-2.5 rounded-xl bg-indigo-600 text-white font-black text-[10px] uppercase shadow-lg shadow-indigo-900/40 hover:bg-indigo-500 transition-all">Save Changes</button>
                        </div>
                    </div>
                )}
            </Modal>
