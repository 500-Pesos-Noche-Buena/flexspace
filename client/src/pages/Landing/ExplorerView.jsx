...
const ExplorerView = () => {
    ...
    const [focusedSpace, setFocusedSpace] = useState(null); // <-- selected space for zoom
    ...
    useEffect(() => {
        ...
        const interval = setInterval(() => fetchExplorerData(false), 3000);
    ...
    const filteredSpaces = useMemo(() => {
        ...
        if (userLatLng) {
            result = result.map(s => ({ ...s, distance: calculateDistance(userLatLng, [s.lat, s.lng]) }))
        ...
        <div className="relative flex flex-col md:flex-row h-[75vh] md:h-162.5 w-full bg-white overflow-hidden md:rounded-[2.5rem] border border-slate-100 shadow-sm">
                ...
                <div className="flex-1 overflow-y-auto p-4 space-y-3 custom-scrollbar">
                    {filteredSpaces.length === 0 ? (
                        ...
                        filteredSpaces.map(s => (
                            <div 
                                key={s._id} 
                                className="group flex gap-4 p-3 hover:bg-indigo-50/50 rounded-2xl transition-all cursor-pointer border border-transparent hover:border-indigo-100"
                                onClick={() => setFocusedSpace(s)} // <-- click to zoom
                            >
            ...
            <main className={`${viewMode === 'list' ? 'hidden md:block' : 'block'} flex-1 relative h-full w-full bg-slate-50 z-10`}>
                <MapExplorer 
                    ...
                    focusedSpace={focusedSpace} // pass focused space to map
                />
            </main>
