...
const WalkinsIndex = () => {
    ...
    const [formData, setFormData] = useState({ space_id: '', name: '', email: '', is_open_time: false });
    const paramsRef = useRef({ page: 1, search: '' });

    const lastFP       = useRef('');      

    // ── FIX 1: stable fetchData via useCallback ──────────────────────────────
    const fetchData = useCallback(async (isSilent = false) => {
        try {
            const res   = await apiGet(`/space/walkins?search=${paramsRef.current.search}`);
            const rows  = res.data || [];

            // ── FIX 2: fingerprint diff — skip setState if data unchanged ────
            ...
            // Keep open checkout modal in sync with latest DB state
    ...
    // ── FIX 3 + 4: visibility guard + mounted flag ───────────────────────────
    useEffect(() => {
        let mounted = true;
    ...
    const handleCheckIn = async (e) => {
        ...
        try {
            ...
            setOpenModal(false);
            setFormData({ space_id: '', name: '', email: '', is_open_time: false });
            fetchData(false);
        } catch {
...
const handleStopTimer = async (id) => {
        try {
            await apiPost(`/space/bookings/${id}/calculate`);
            fetchData(false);
            showToast({ icon: 'success', title: 'Session frozen' });
        } catch (e) {
 ...
 const handleFinalCheckout = async (paymentDetails) => {
        ...
        try {
            const res = await apiPost(`/space/bookings/${checkoutSession._id}/checkout`, {
    ...
    <div className="relative">
        ...
        {showSuggestions && guestSuggestions.length > 0 && (
            <div className="absolute z-50 top-full left-0 right-0 mt-1 bg-[#1a1a1f] border border-white/10 rounded-2xl overflow-hidden shadow-2xl">
                <p className="text-[9px] font-black uppercase tracking-widest text-slate-600 px-4 pt-3 pb-1">
                    Past Guests
                </p>
                {guestSuggestions.map((guest, i) => (
                    ...
                    <div className="flex items-center justify-between p-4 bg-white/5 border border-white/10 rounded-2xl">
                        <span className="text-[10px] font-black text-white uppercase tracking-widest">Open Time / All Day</span>
                        <input 
                            type="checkbox" 
